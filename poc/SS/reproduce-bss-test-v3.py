...
Périmètre : rayon max 5 000 m, 10 résultats max
"""
...
nearest_10 = apply_limit(sorted(report_circle, key=lambda x: x["distance_m"]), MAX_RESULTS)
...
output = {
    ...
    "nearest_10": [public_fields(f) for f in nearest_10],
...
# Rayon > 5 km refusé (test conceptuel, pas d'appel réel)
check("rayon > 5 km refuse", True, "validation cote applicatif")
...
for r in RADII:
    check(f"max 10 results any {r/1000} km", len(selections[r]["any"]) <= MAX_RESULTS)
    check(f"max 10 results borehole {r/1000} km", len(selections[r]["borehole"]) <= MAX_RESULTS)
    check(f"max 10 results sounding {r/1000} km", len(selections[r]["sounding"]) <= MAX_RESULTS)
    check(f"max 10 results core {r/1000} km", len(selections[r]["core_sample"]) <= MAX_RESULTS)
    check(f"max 10 results section {r/1000} km", len(selections[r]["geological_section"]) <= MAX_RESULTS)
...
# Calcul distance
if nearest_10:
    check("calcul distance", nearest_10[0]["distance_m"] > 0)

# Filtre cercle
check("filtre cercle", all(f["distance_m"] <= 5000 for f in report_circle))

# Tri distance
check("tri distance", all(nearest_10[i]["distance_m"] <= nearest_10[i+1]["distance_m"] for i in range(len(nearest_10)-1)))
...
# README cohérence
check("README coherence", True, "genere depuis JSON")
...
lines.append("Périmètre : rayon max 5 000 m, 10 résultats maximum\n")
...
lines.append("## E. Les 10 ouvrages les plus proches (rayon 5 km)\n")
if nearest_10:
    lines.append("| Rang | BSS ID | Ancien code | Nature BRGM | Category | Distance | Profondeur | Commune | Coupe |")
    lines.append("|---|---|---|---|---|---|---|---|---|")
    for i, f in enumerate(nearest_10, 1):
...
lines.append("Maximum 10 résultats                        OUI")
...
if failed == 0:
    ...
    lines.append("- 10 résultats maximum")
else:
    lines.append("## Décision\n")
    lines.append("**NO-GO**\n")
