...
def read_temperature(path: Path) -> pd.Series:
    ...
    if index.isna().any():
        raise SnapshotError("Horodatage invalide dans era5-land.csv")
    if values.isna().any():
        raise SnapshotError("Valeur de température invalide dans era5-land.csv")
    series = pd.Series(values.to_numpy(), index=index).sort_index()
    if series.index.has_duplicates:
        raise SnapshotError("Horodatages dupliqués dans era5-land.csv")
