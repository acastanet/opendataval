...
def date_to_noleap_doy(value: date) -> int:
    """Projette une date civile sur le calendrier no-leap de 365 jours.

    ``_CUMULATIVE`` est déjà construit avec un février de 28 jours : une fois le
    29 février écarté, la somme donne directement le rang no-leap, sans
    correction supplémentaire pour les années bissextiles.
    """
    if value.month == 2 and value.day == 29:
        raise ValueError("Le 29 février n'existe pas dans le calendrier no-leap")
    return _CUMULATIVE[value.month - 1] + value.day
