...
_SIGNAL_CONFIG_BOUNDARIES = (*_BOUNDARY_FIELDS, "summer_length")
...
_CAVEATS = {
    ...
    "smoother-sensitivity-above-threshold": (
        "Au moins une décennie dépasse le seuil de robustesse de 5 jours entre les deux lissages : le sens du "
        "déplacement reste exploitable, son amplitude exacte doit être présentée avec réserve."
    ),
    "flat-winter-curve-amplification": (
        "Près du minimum hivernal la courbe thermique est plate : un même réchauffement y déplace la frontière de "
        "beaucoup plus de jours qu'en été. Les écarts du printemps et de l'hiver ne traduisent donc pas une saison "
        "davantage touchée."
    ),
}
...
def _comparison(early: Mapping[str, Any], late: Mapping[str, Any]) -> dict[str, float | None]:
    """Compare les deux décennies dès lors qu'aucune n'est rejetée.

    La règle QA distingue trois niveaux : au-delà de 5 jours d'écart entre les
    deux lissages le résultat est « à documenter » (``partial``), et seul un
    écart supérieur à 10 jours le rend inexploitable (``rejected``). Supprimer la
    comparaison dès ``partial`` reviendrait à appliquer le seuil de rejet au
    seuil d'alerte.
    """
    early_values = early["canonical_boundaries"]
    late_values = late["canonical_boundaries"]
    if early["status"] == "rejected" or late["status"] == "rejected":
        return {field: None for field in _SIGNAL_CONFIG}
    if any(values.get(field) is None for values in (early_values, late_values) for field in _SIGNAL_CONFIG_BOUNDARIES):
        return {field: None for field in _SIGNAL_CONFIG}
    return {
...
def build_v4_signals(data: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    sensitivity_flagged = any(decade["status"] == "partial" for decade in (early, late))
    ...
    for field, (definition_id, boundary, mode) in _SIGNAL_CONFIG.items():
        ...
        if field in ("spring_start_shift_days", "winter_start_shift_days"):
            caveats.append("flat-winter-curve-amplification")
        if sensitivity_flagged:
            caveats.append("smoother-sensitivity-above-threshold")
        ...
        signals.append(
            {
                ...
                "caveat_ids": caveats,
                "quality_status": "partial" if sensitivity_flagged else "valid",
                "metadata": {
...
def build_climate_result_v4(series: ThermalSeasonsInput, *, context: ResultContext) -> dict[str, Any]:
    ...
    if not quality["comparison_available"] or len(signals) != 5:
        status = "insufficient"
    elif quality["decades_ok"] == 2:
        status = "valid"
    else:
        # Une décennie dépasse le seuil de robustesse de 5 jours sans atteindre
        # celui de rejet : le résultat est diffusable, accompagné de son caveat.
        status = "partial"
    caveat_ids: list[str] = []
