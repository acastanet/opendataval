...
function endpoint(extension, latitude, longitude, download = false) {
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    const response = await fetch(endpoint("svg", latitude, longitude));
    if (!response.ok) {
    ...
    downloadSvg.href = endpoint("svg", latitude, longitude, true);
    downloadPng.href = endpoint("png", latitude, longitude, true);
