...
### Pourquoi ne pas colorer directement le percentile ? (V4)

Les six lignes n’ont pas les mêmes unités ni les mêmes distributions. Il faut donc une
grandeur adimensionnelle. Le percentile en est une, et les cinq classes ci-dessus continuent
de s’appuyer sur lui — mais il s’est révélé un mauvais support pour la **couleur**, pour deux
raisons.

1. **Le rang est uniforme par construction.** Une année banale mais du bon côté de la médiane
   se retrouve déjà teintée. La moitié de la matrice se colore sans qu’il se soit rien passé,
   et le rouge perd sa valeur d’alerte.
2. **Le rang sature.** Toute année dépassant le record 1991–2020 vaut `P100`. Sur la
   température du point de démonstration, 2020, 2022, 2023, 2024 et 2025 sont toutes à `P100`
   alors qu’elles s’étalent de +1,0 à +2,8 σ : cinq années d’intensité très différente
   sortaient d’un rouge identique.

La V4 colore donc un **écart standardisé robuste**, borné à ±3 σ :

```text
sigma    = (P90 − P10) / 2,563          # écart P10–P90 d'une loi normale, en σ
z        = (valeur − P50) / sigma
position = 0,5 + z / 6                  # 0 à ±3 σ, borné dans [0 ; 1]
```

Cette position brute est ensuite passée dans une **courbe d’accentuation** qui écrase la
variabilité courante vers le blanc et dilate les queues :

```text
écart   = position − 0,5
finale  = 0,5 + signe(écart) × (|écart| / 0,5)² / 2
```

Calibrage retenu : 1 σ (environ une année sur six) reste blanc, 2 σ se voit nettement, 3 σ
sature la palette. Une année ne se colore que si elle est réellement exceptionnelle.

Le percentile reste le **repli** lorsque la référence est trop courte ou dégénérée
(`P90 = P10`, typique des séries de comptages majoritairement nulles) ; le garde-fou sur les
ex æquo continue alors de s’appliquer.

La couleur représente donc :

> **« De combien cette année s’écarte-t-elle du climat de référence de ce même indicateur,
> et cet écart est-il exceptionnel ? »**
...
## Ce que la V4 a finalement retenu

Cette section décrit l’intention initiale. À l’usage, six palettes distinctes obligeaient le
lecteur à réapprendre le code couleur à chaque ligne, ce qui rendait toute lecture verticale
— et donc tout indice de synthèse — impossible.

La V4 revient à **une palette unique divergente** (ColorBrewer RdBu, zone claire élargie),
sous une convention explicite et invariable :

> **bleu = moins de la variable, rouge = plus de la variable**, y compris pour les
> précipitations.

L’ambiguïté redoutée ci-dessus est levée par deux moyens qui n’existaient pas dans la V3 :

- l’intitulé de chaque ligne et son infobulle nomment la variable et le sens de son
  augmentation, si bien qu’un rouge sur « Précipitations » se lit « plus de pluie » et jamais
  « sec » ;
- l’échelle accentuée décrite en §4 réserve la couleur franche à l’exceptionnel, ce qui limite
  fortement le nombre de cellules colorées à interpréter.

Les palettes par indicateur restent sérialisées dans le JSON (champ `palette` de chaque ligne)
pour un consommateur tiers, mais **le rendu de référence ne les utilise pas**.

## Deux habillages, un seul jeu de couleurs

La couleur d’une cellule est un contrat : elle ne dépend que de la donnée. En revanche le
**fond** sur lequel les bandes reposent relève de la mise en page, et deux versions sont
publiées à partir du même calcul :

| Thème | Fond | Bandes |
| --- | --- | --- |
| `light` | `#FAFAF7` | posées à plat, comme la V4 initiale |
| `neutral` | `#C5C4C1` | plaque `#FBFAF7` légèrement ombrée sous chaque ligne |

Sur fond clair, une bande majoritairement blanche se confond avec la page ; le fond neutre lui
redonne un contour sans ajouter la moindre bordure ni le moindre trait. C’est aussi ce qui a
permis de supprimer le filet horizontal qui détachait la ligne « Empreinte bilan » : un
interligne élargi et, sur fond neutre, le relief suffisent.

L’ombre est portée par une plaque pleine dessinée sous la bande plutôt que par un filtre
appliqué au groupe des trente cellules : le rendu ne dépend alors pas de la façon dont le
