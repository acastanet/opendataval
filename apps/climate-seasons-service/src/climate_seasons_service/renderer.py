...
La composition reproduit le rendu V5 neutral du POC validé.
"""
...
def render_thermal_seasons_svg(document: Mapping[str, Any]) -> str:
    """Rend le payload scientifique thermal-seasons V1 sans aucun recalcul."""
    ...
    parts.append('<text x="40" y="92" class="meta">1996–2025 · référence 1991–2020 · seuils T25 / T75 locaux</text>')
    ...
    for label, decade, y in bands:
        ...
        for seg, boundary in enumerate(boundaries):
            if not isinstance(boundary, Mapping):
                continue
            med = boundary.get("median")
            p25 = boundary.get("p25")
            p75 = boundary.get("p75")
        ...
        for seg in range(5):
            ...
            seg_start = edges[seg]
            seg_end = edges[seg + 1]
            center_x = (_x(seg_start) + _x(seg_end)) / 2
            parts.append(
    ...
    if isinstance(comparison, Mapping):
        sig_y = EARLY_TOP + 17
        change = comparison.get("summer_length_change_days")
