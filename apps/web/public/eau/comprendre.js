...
(function () {
  ...
  function afficherTorrent(mesures) {
    ...
    var etatEl = document.getElementById("torrent-etat");
    var liveEl = document.getElementById("torrent-live");
    if (etatEl) etatEl.hidden = true;
    if (liveEl) liveEl.hidden = false;

    var valeurEl = document.getElementById("torrent-valeur");
    if (valeurEl) valeurEl.textContent = C.formaterHauteur(derniere.y);
    ...
    var metaEl = document.getElementById("torrent-meta");
    ...
    if (rang != null) {
      ...
      var syntheseEl = document.getElementById("torrent-synthese");
      if (syntheseEl) syntheseEl.innerHTML =
      ...
      var curseur = document.getElementById("jauge-curseur");
      if (curseur) curseur.style.left = Math.round(rang * 100) + "%";
    }

    var grapheEl = document.getElementById("torrent-graphe");
    if (grapheEl) {
      var graphe = Graphe.creer(grapheEl, {
      ...
      var legendeEl = document.getElementById("torrent-legende");
      if (legendeEl) legendeEl.textContent =
  ...
  async function chargerNappe() {
    ...
    try {
      ...
      etat.hidden = true;
      var liveEl = document.getElementById("nappe-live");
      if (liveEl) liveEl.hidden = false;
      var valeurEl = document.getElementById("nappe-valeur");
      if (valeurEl) valeurEl.textContent = C.formaterNiveauNappe(stats.derniere.niveau_m_ngf);
      if (stats.derniere.profondeur_m != null) {
        var profEl = document.getElementById("nappe-profondeur");
        if (profEl) profEl.textContent = "(nappe à " + C.formaterProfondeur(stats.derniere.profondeur_m) + ")";
      }
      var dateEl = document.getElementById("nappe-meta");
      if (dateEl) dateEl.textContent = "le " + C.formaterDateCourte(stats.derniere.date);

      if (stats.situation) {
        var bloc = document.getElementById("nappe-situation");
        if (bloc) {
          bloc.hidden = false;
          var pastille = document.getElementById("nappe-pastille");
          if (pastille) pastille.style.background = C.COULEUR_SITUATION[stats.situation.classe];
          var sitTxt = document.getElementById("nappe-situation-txt");
          if (sitTxt) sitTxt.textContent =
            "Niveau " + stats.situation.libelle + " pour un mois de " + C.formaterMois(stats.derniere.date) + ".";
        }
      }

      var minmaxEl = document.getElementById("nappe-minmax");
      ...
      // Graphe 13 mois.
      var grapheEl = document.getElementById("nappe-graphe");
      if (grapheEl) {
        var graphe = Graphe.creer(grapheEl, {
        ...
        var legendeEl = document.getElementById("nappe-legende");
        if (legendeEl) legendeEl.textContent =
