# Gateway Service

> Point d'entrée unique des API v2 (`/api/v2/*`). Aucune logique métier, aucun accès base.
> Dernière mise à jour : 2026-07-24 · Dernière vérification : 2026-07-24
> Code : `apps/gateway-service/`

## Rôle

Le gateway est la façade des nouvelles API OpenDataVal. Il valide sa configuration au démarrage, expose les contrôles de santé, propage `x-request-id`, normalise les erreurs d'infrastructure et route vers les services v2 (ou vers le monolithe via un pont temporaire). Il ne doit jamais devenir un monolithe métier : chaque nouvel endpoint est ajouté domaine par domaine avec un contrat explicite.

## Périmètre / endpoints

| Route publique | Cible | Méthodes |
|---|---|---|
| `GET /health` | statut du processus | GET |
| `GET /ready` | vérifie aussi que l'API historique répond sur `/api/health` | GET |
| `GET /api/v2/gateway` | statut du gateway | GET |
| `GET /api/v2/geography/resolve` | `geography-service:/internal/v1/geography/resolve` | GET |
| `GET /api/v2/weather/temperature` | `weather-service:/internal/v1/weather/temperature` | GET |
| `GET /api/v2/vigilance` | `weather-vigilance-service` après résolution du département | GET |
| `GET /api/v2/fire/nearby` | `fire-detection-service:/v1/fire/nearby` avec 50 km / 7 jours imposés | GET |
| `/api/v2/legacy/*` | `api:/api/*` (pont) | GET, HEAD |

Format d'erreur unifié : `{ error: { code, message, retryable }, requestId }`.

Le pont legacy accepte uniquement `GET`/`HEAD`, transmet le `x-request-id`, applique un délai maximal, ne suit pas les redirections, retire les en-têtes de proche en proche, refuse les traversées de chemin et garantit que la cible reste sous le préfixe `/api` du monolithe.

## Dépendances

- `api` (monolithe) — cible du pont legacy et de la sonde `/ready` ;
- `geography-service`, `weather-service`, `weather-vigilance-service`, `fire-detection-service` — cibles des proxys v2 ;
- aucune base de données, aucun cache, aucune file de messages.

## Configuration (`src/config.ts`)

| Variable | Défaut | Description |
|---|---|---|
| `HOST` | `0.0.0.0` | Adresse d'écoute |
| `PORT` | `3000` | Port HTTP interne |
| `LEGACY_API_URL` | `http://api:3000` | API historique |
| `GATEWAY_UPSTREAM_TIMEOUT_MS` | `5000` | Délai max du pont legacy |
| `GEOGRAPHY_SERVICE_URL` | `http://geography-service:3000` | Cible geography |
| `GEOGRAPHY_SERVICE_TIMEOUT_MS` | `3000` | Délai geography |
| `WEATHER_SERVICE_URL` | `http://weather-service:3000` | Cible weather |
| `WEATHER_SERVICE_TIMEOUT_MS` | `3000` | Délai weather |
| `VIGILANCE_SERVICE_URL` | `http://weather-vigilance-service:3000` | Cible vigilance |
| `VIGILANCE_SERVICE_TIMEOUT_MS` | `3000` | Délai vigilance |
| `FIRE_DETECTION_SERVICE_URL` | `http://fire-detection-service:3000` | Cible détection incendie |
| `FIRE_DETECTION_SERVICE_TIMEOUT_MS` | `20000` | Délai agrégé FIRMS/EUMETSAT |
| `APP_VERSION` | `dev` | Version exposée par les routes de santé |

## Lancement & contrôles

```bash
pnpm install --frozen-lockfile
pnpm check:gateway
docker compose build gateway fire-detection-service
docker compose up -d gateway fire-detection-service
curl -i http://localhost:8080/api/v2/gateway
curl -i 'http://localhost:8080/api/v2/fire/nearby?lat=44.0812&lon=3.6415'
```

Le `Caddyfile` route déjà tout `/api/v2/*` vers le gateway : l'ajout du lot 5 ne nécessite pas de nouvelle règle Caddy.

`/health` confirme que le processus tourne ; `/ready` vérifie en plus que l'API historique répond sur `/api/health`. Les tests automatisés couvrent la validation, le routage, la propagation du `request-id` et les délais d'attente.

## Rollback

Retirer le service et le proxy v2 : les routes historiques `/api/*` sont indépendantes et restent servies.

## Limites volontaires

Pas encore d'authentification, de Redis/file de messages, de gRPC, de circuit breaker avancé, ni de logique métier dans le gateway. La fixation du rayon et de l'historique de la route feu est un contrôle de contrat, pas un calcul de détection.

## Docs liées

- Architecture globale : [`../../architecture/ARCHITECTURE-GENERALE.md`](../../architecture/ARCHITECTURE-GENERALE.md)
- Détection incendie : [`../fire-detection/README.md`](../fire-detection/README.md)
