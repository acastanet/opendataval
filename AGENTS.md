...
`site-service` (`apps/site-service`, MVP work tracked in `agent/mvp/`) is the OpenDataVdA tile orchestrator: it is not yet part of the JSON API surface above, but its create/build routes are proxied unauthenticated at `POST /api/v2/sites` and `POST /api/v2/sites/:tileId/build` (`apps/gateway-service/src/site-proxy.ts`, ADR-009 in `agent/mvp/09-DECISIONS.md` — supersedes the earlier internal-only ADR-006). Its own internal routes stay at `/internal/v1/sites/*`. The gateway serves reads as an HTML page at `GET /api/v2/sites/:tileId` (`apps/gateway-service/src/pages/site-instance.ts`), not as JSON.
